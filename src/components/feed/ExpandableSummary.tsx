...
export const ExpandableSummary = ({
  ...
  const displayContent = translatedContent || card.fullContent || card.summary;
  return <div className="relative">
      ...
      <div onClick={handleExpand} className="cursor-pointer group">
        <p className={cn("text-card-summary text-white mb-3 transition-all duration-300 font-sans font-normal", !isExpanded && "line-clamp-3")}>
          {isExpanded ? displayContent : card.summary}
        </p>
      ...
      {/* Expanded content overlay for long content */}
      {isExpanded && displayContent.length > 500 && <div className="mt-2 max-h-[40vh] overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent pr-2">
